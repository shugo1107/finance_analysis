...
class AI(object):
    ...
    def get_current_position(self):
        ...
        for product_code in self.product_codes:
            for signal in ['ADX', 'ATR', 'EMA']:
                if self.position[product_code][signal]:
                    requests.post(settings.WEB_HOOK_URL, data=json.dumps({
    ...
    def can_buy(self, candle, units, product_code=None, fx_adjustment=1):
        if product_code is None:
            product_code = self.product_code
        ...
        #     return True
                ...
                units * candle.close * fx_adjustment / self.leverage + float(self.balance.require_collateral):
    ...
    def can_sell(self, candle, units, product_code=None, fx_adjustment=1):
        if product_code is None:
            product_code = self.product_code
        if product_code not in constants.TRADABLE_PAIR:
    ...
    def trail_buy(self, candle, units, product_code=None, trade_signal=None, fx_adjustment=1, stop_loss=None,
        ...
        if not self.can_buy(candle, units, product_code, fx_adjustment):
        ...
        if not trade:
            return False
        require_collateral = units * float(trade.price) * fx_adjustment / self.leverage
    ...
    def trail_sell(self, candle, units, trade_signal=None, fx_adjustment=1, stop_loss=None,
        ...
        if not self.can_sell(candle, units, product_code, fx_adjustment):
    ...
    def atr_trade(self, indicator, product_code, fx_adjustment):
        ...
        # 買いのときの手動で出すSLの更新
        if positions and positions[0].side == "BUY" and indicator['atr_up_2'][-1] > positions[0].stop_loss:
        ...
        # 売りのときの手動で出すSLの更新
        if positions and positions[0].side == "SELL" and indicator['atr_down_2'][-1] < positions[0].stop_loss:
        ...
        if not positions and indicator['atr_down'][-2] < indicator['df'][-2].close \
